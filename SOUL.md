...
So every PDF in `knowledge/` ships with a committed, pre-extracted `.txt` sibling (generated by `knowledge/refresh_txt.sh`). Search those:
...
Grep the `.txt`, read the matching section, answer from it, and cite the source document by name. Only use the `read-document` skill for documents that *aren't* pre-extracted — files the user uploads mid-conversation, or PDFs you download.
